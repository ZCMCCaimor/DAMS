<div class="container p-3">
    @if(count($myappointment)>=1)
    @foreach ($myappointment as $row)
           ...
           <div class="card-body text-secondary" style="font-size:14px">
               ...
               <h6 class="text-dark">
                @foreach ($doctor as $doc)
                    To : Dr. <span style="text-transform: uppercase">{{$doc->name}}</span>
                    <br>
                    <span style="font-size:13px">{{$doc->email}} | #{{$doc->contactno}}</span>
                    <br>
                    <span style="font-size:13px">{{$doc->address}}</span>
                @endforeach
               </h6>
